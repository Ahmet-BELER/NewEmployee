...
const EmployeeContextProvider = (props) => {
    ...
    const [employees, setEmployees] = useState([
        ...
        {id:uuidv4(), name: 'Ahmet', email: 'Ahmet@mail.com', address: 'Via Monte Bianco 34, Turin, Italy', phone: '(480) 631-2097'}
    ...
    const addEmployee = (name, email, address, phone) => {
        setEmployees([...sortedEmployees, {id:uuidv4(), name, email, address, phone}])
    }
   const deleteEmployee = (id)=> {
setEmployees(sortedEmployees.filter(employee => employee.id !== id))
   }
   const updateEmployee = (id, updatedEmployee) => {
    setEmployees(sortedEmployees.map((employee) => (employee.id === id ? updatedEmployee : employee)))}
